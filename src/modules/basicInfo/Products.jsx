...
import product_categories from '../../resources/product_categories.json';
...
import ProductAddEditModal from './modals/ProductsAddEditModal';
...
const Products = () => {
  ...
  const loadProductCategories = async () => {
    ...
    setCategoryList(product_categories);
    setOnSelect(product_categories[0].id);
    const tempCategoryList = product_categories;
  ...
  const toggleCreateProductModal = () => {
    setCreateProductModal((prev) => !prev);
  };
  ...
  const handleEditClick = async (currentRowData) => {
    setCurrentItemRowData(currentRowData);
    toggleEditProductModal();
  };

  const makeProductCategoryOptions = () => {
    return [
      { value: 0, label: 'Add new category' },
      ...removedCategoryList.map((item) => {
        return {
          value: item.id,
          label: selectedLanguage === 'en' ? item.en_name : item.jp_name,
        };
      }),
    ];
  };
    ...
    <div className="products">
      <div className="product-tab">
                  ...
                  {selected === 1 ? (
                    <div className="photo-upload-portion">
                      <div className="photo-border-portion">
                        ...
                        >
                          {t('basic_info:PRODUCTS.ADD_PRODUCT_LABEL')}
                        </button>
                      </div>
                    </div>
                  ) : (
        ...
        <ProductAddEditModal
          ...
          categories={makeProductCategoryOptions()}
          edit={false}
        ...
        <CommonDeleteModal
          ...
          list={productItems}
          currentRow={currentItemRowData}
          deleteId={currentItemRowData.id}
        />
      </div>
    </div>
  );
